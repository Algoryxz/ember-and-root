...
> In this repository, UI/UX intelligence and reasoning must resolve in this order:
> 1. `AGENTS.md`
...
> 4. Generic reusable design/frontend skills (`ui-ux-pro-max`, `frontend-design`, etc.)
>
> **CRITICAL PROJECT CONSTRAINTS (OVERRIDE GENERIC RECOMMENDATIONS):**
> - **Product Identity**: "Illuminated field journal" (charcoal, parchment, copper, sage).
> - **Forbidden Aesthetics**: No generic SaaS dashboards, default shadcn looks, glassmorphism everywhere, blue/purple AI gradients, enterprise sidebars, or arbitrary color schemes.
...
> - **Root Visualization Rules**:
>   - Root geometry is fixed authored SVG geometry with state-controlled path reveal.
>   - NEVER use procedural graph layouts, force graphs, free pan/zoom, giant graph packages, SVG morph engines, Three.js, Rive, Lottie, or GSAP unless explicitly approved.
>   - Interactive nodes must be accessible HTML controls positioned over/around the SVG. Provide a semantic list fallback.
> - **Dependencies**: Never add npm/python dependencies without delivery owner approval.
> - **Responsive / Mobile**: First-class acceptance criteria (~320px minimum phone, ~375px phone, ~768px tablet, ~1440px desktop, 200% zoom).
