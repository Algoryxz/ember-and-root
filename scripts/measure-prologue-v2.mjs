...
try {
  for(const profile of [{name:'desktop',width:1440,height:900,dpr:1,cpu:1},{name:'mobile-emulation',width:390,height:844,dpr:2,cpu:4}]){
    ...
    await page.evaluate(()=>{
      ...
      const tick=time=>{const s=window.__frameSample;if(s.last)s.frames.push({dt:time-s.last,scene:document.querySelector('.shore-sequence') ? 'shore-'+document.querySelector('.shore-sequence').dataset.beat : document.querySelector('main').dataset.scene});s.last=time;s.handle=requestAnimationFrame(tick);};
    ...
    const stages={};
    for(const scene of ['forest','guided','chamber','discovery','surge','shore-0','shore-1','shore-2','shore-3','shore-4']){
  ...
  await writeFile('docs/review/prologue-v2/performance.json',JSON.stringify(results,null,2));console.log(JSON.stringify(results,null,2));
